...
<!--
To fix on a reflow of the examples:

No `type=` on textarea
The `id="review"` for a review should say `body`

id param in route should be an int, can we do that with React Router

Make the add-review form its own component.

`for=` to `htmlFor=`

Always use a `fetchThing` method and use async/await. This will also make error handling easier. Though we will have to deal with useEffect dependencies

Test for `2xx` responses instead of error codes. This will make error handling easier

Make Summary and Body required on Review and add the error handling
